...
public:
	...
	void value(std::string_view key, std::string& ret, std::string_view = "")
			const;
	...
	void value(
			std::string_view key, std::filesystem::path& ret,
			std::string_view defaultvalue = "") const {
		std::string val;
		value(key, val, defaultvalue);
		ret = std::filesystem::u8path(val);
	}
	...
	// Only want this for std::filesystem::path, but implicit conversions make
	// this annoying.
	template <
			typename Path,
			std::enable_if_t<std::is_same_v<Path, std::filesystem::path>>>
	void set(std::string_view key, Path&& value, bool write_out) {
		set(key, std::forward<Path>(value).u8string(), write_out);
	}
